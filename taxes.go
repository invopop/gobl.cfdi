...
func newImpuestos(totals *bill.Totals, currency *currency.Code, regime *tax.RegimeDef) *Impuestos {
...
func newImpuesto(rate *tax.RateTotal, currency *currency.Code) *Impuesto {
	...
	if rate.Percent == nil {
		return &Impuesto{
			...
			Impuesto:   rate.Ext[addon.ExtKeyTaxType].String(),
...
func newConceptoImpuestos(line *bill.Line, regime *tax.RegimeDef) *ConceptoImpuestos {
	var traslados, retenciones []*Impuesto
...
func newConceptoImpuesto(line *bill.Line, tax *tax.Combo) *Impuesto {
	if tax.Percent == nil {
		return &Impuesto{
			...
			Impuesto:   tax.Ext[addon.ExtKeyTaxType].String(),
			TipoFactor: TipoFactorExento,
		}
	}
